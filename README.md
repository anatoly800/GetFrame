...
Приложение для Android (реализация UI на Avalonia .NET 9) 
...
- Добавлен каркас Avalonia-приложения с окном из трёх зон (`Auto, *, Auto`) и скруглёнными рамками.
- Добавлена VM-логика: Open/Save/Cancel, автопереход на последний кадр, debounce 800мс, статус `Idle`, отображение elapsed.
- Добавлен Android-сервис на `MediaMetadataRetriever` с обработкой API `< 28` и `>= 28`.
- Сохранение выбранного кадра в PNG.

> Для локального запуска Open-команда в текущем каркасе читает путь из переменной окружения `GETFRAME_SAMPLE_VIDEO`.
